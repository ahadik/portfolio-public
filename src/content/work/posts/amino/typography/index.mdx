...
#### Requirements
Kicking off this effort, I wanted to have some clear goals to guide my work. For the most part, I borrowed from the Amino Colorway requirements:
...
##### Scale
I knew that there was no reason to re-invent the wheel on our typography. To begin, I settled on the standard 16px base font size. When deciding how I wanted to scale up and down from this base, I knew that I wanted a system I could compute in code, instead of managing manually. This wasn't quite as easy as I imagined though.

My first challenge was determining how the scale would expand and contract. For sizes smaller than 16px, I wanted the scale to shrink linearly, for an even downwards progression. However for the larger sizes, I wanted the scale to increase exponentially, so that each step up in the scale grew by more than the last. This allowed each step in the scale to have a unique purpose, visually distinct from the others.

<Image name="scale" />

To achieve this effect, I arrived at a universal multiplier of 0.125 that would be applied differently moving up and down the scale. The scale is implemented in Sass as follows:
...
- Default: Sets the type to Strateos's body font, Atlas.
- Branded: Sets the type to our Strateos brand font Axia.
...
All told, the process took a week or so, and in the end, we had a clear, consistent typographic system. Feedback was overwhelmingly positive. Most users, internal and external, recognized that the text in the app became more legible and consistent.

Since its inception, this typography system has proved resilient, needing very few updates. When updates were necessary, the changes could be applied across the entire app at once - a trivial benefit to web development, but never before achieved at Transcriptic.
